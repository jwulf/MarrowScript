...
// Handler for ItemTraded
export async function notifyItemTraded(event: SystemEvent, recipientEmail: string): Promise<void> {
  const __payload = escapeHtml(JSON.stringify(event.payload, null, 2));
  await sendEmail({
    to: recipientEmail,
    subject: "ItemTraded notification",
    body: `<p>Event <strong>${escapeHtml("ItemTraded")}</strong> occurred.</p><pre>${__payload}</pre>`,
  });
}

// Handler for PlayerLeveledUp
export async function notifyPlayerLeveledUp(event: SystemEvent, recipientEmail: string): Promise<void> {
  const __payload = escapeHtml(JSON.stringify(event.payload, null, 2));
  await sendEmail({
    to: recipientEmail,
    subject: "PlayerLeveledUp notification",
    body: `<p>Event <strong>${escapeHtml("PlayerLeveledUp")}</strong> occurred.</p><pre>${__payload}</pre>`,
  });
}

// Handler for TradeOfferCreated
export async function notifyTradeOfferCreated(event: SystemEvent, recipientEmail: string): Promise<void> {
  const __payload = escapeHtml(JSON.stringify(event.payload, null, 2));
  await sendEmail({
    to: recipientEmail,
    subject: "TradeOfferCreated notification",
    body: `<p>Event <strong>${escapeHtml("TradeOfferCreated")}</strong> occurred.</p><pre>${__payload}</pre>`,
  });
}

export function registerNotificationHandlers(eventBus: any): void {
  // TODO: implement recipient lookup per event type
  // eventBus.subscribe("ItemTraded", async (event: SystemEvent) => {
  //   const recipientEmail = await lookupRecipient(event);
  //   await notifyItemTraded(event, recipientEmail);
  // });
  // eventBus.subscribe("PlayerLeveledUp", async (event: SystemEvent) => {
  //   const recipientEmail = await lookupRecipient(event);
  //   await notifyPlayerLeveledUp(event, recipientEmail);
  // });
  // eventBus.subscribe("TradeOfferCreated", async (event: SystemEvent) => {
  //   const recipientEmail = await lookupRecipient(event);
  //   await notifyTradeOfferCreated(event, recipientEmail);
  // });
}
